...
dataset.load_file("./DataSets/NALinearSphereScattersDataSet.npz")#, limit=1024*1024)
print('Loaded data... '+str(dataset.data.shape))


def test_dataset(sigma, sigmax, sigmay, sigmaz, g):
    test_data = dataset.get_filtered_data({
        'sigma': (sigma - 3, sigma + 4),
        'sigmax': (sigmax - 0.2, sigmax + 0.2),
        'sigmay': (sigmay - 0.2, sigmay + 0.2),
        'sigmaz': (sigmaz - 0.2, sigmaz + 0.2),
    ...
    sx_x = test_data[:, 7].cpu().numpy()
    sx_y = test_data[:, 8].cpu().numpy()
    plt.hist2d(sx_x, sx_y, density=True, bins=30)


# test_dataset(4.0, 1.0, 0.0, 0.0, 0.9)
# plt.show()
# exit()
...
state = vae.training.start_training(path, dataset, cvae_factory, batch_size=64*1024, epochs=2000)
